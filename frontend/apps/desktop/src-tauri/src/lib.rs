...
fn unread_tooltip(count: u32) -> String {
    if count == 0 {
        ...
        format!("WebTui Chat - {count} th\u{00f4}ng b\u{00e1}o ch\u{01b0}a \u{0111}\u{1ecd}c")
    }
}
...
fn install_tray(app: &mut tauri::App) -> tauri::Result<()> {
    let show = MenuItem::with_id(app, "show", "Hi\u{1ec7}n WebTui Chat", true, None::<&str>)?;
    let hide = MenuItem::with_id(
        app,
        "hide",
        "\u{1ea8}n c\u{1eed}a s\u{1ed5}",
        true,
        None::<&str>,
    )?;
    let quit = MenuItem::with_id(app, "quit", "Tho\u{00e1}t", true, None::<&str>)?;
...
mod tests {
    ...
    fn unread_tooltip_includes_unread_count() {
        assert_eq!(
            unread_tooltip(7),
            "WebTui Chat - 7 th\u{00f4}ng b\u{00e1}o ch\u{01b0}a \u{0111}\u{1ecd}c"
        );
    }

    #[test]
    fn local_tauri_config_has_valid_updater_plugin_config() {
        let config: serde_json::Value =
            serde_json::from_str(include_str!("../tauri.conf.json")).expect("valid Tauri config");
        let updater_config = config
            .pointer("/plugins/updater")
            .expect("plugins.updater config exists")
            .clone();
