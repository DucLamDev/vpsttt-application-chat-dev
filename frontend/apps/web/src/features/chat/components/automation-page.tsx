...
import type { CronJob, IncomingWebhook, JsonObject, OutgoingWebhook } from "@webtui/types";
...
export function AutomationPage({
  ...
  const updateJobMutation = useMutation({
    mutationFn: ({ job, status }: { job: CronJob; status: string }) => api.cronjobs.update(workspaceId as string, job.id, {
...
function JobsView({ createMutation, deleteMutation, isCreateOpen, jobs, jobsQuery, onCreateOpen, onSelect, runMutation, runs, selectedJob, updateMutation }: {
  ...
  selectedJob?: CronJob;
  updateMutation: ReturnType<typeof useMutation<unknown, Error, { job: CronJob; status: string }>>;
}) {
  return (
    <>
...
function WebhooksView({ channelOptions, createIncomingMutation, createOutgoingMutation, deliveries, incoming, incomingQuery, isCreateOpen, onCreateOpen, onSelectOutgoing, outgoing, outgoingQuery, selectedOutgoing }: {
