...
ZEGO_APP_SIGN=THAY_BANG_APP_SIGN_TU_ZEGOCLOUD
ZEGO_SERVER_SECRET=THAY_BANG_SERVER_SECRET_TU_ZEGOCLOUD
ZEGO_TOKEN_TTL=24h
```

Lấy `ZEGO_APP_ID`, `ZEGO_APP_SIGN`, `ZEGO_SERVER_SECRET` trong ZEGOCLOUD Console của project Voice & Video Call. Chỉ đặt các giá trị này trong `.env` trên VPS hoặc secret runtime tương đương; không commit secret thật.
