# Download Page Spec

The public Android download entrypoint should live at `download.vpsttt.com`.
This page is the first distribution surface while Google Play is prepared later.

## Source Metadata

The page must read release metadata generated by M11:

- `mobile-release-manifest.json`
- `app-prod-release.apk.sha256`
- `app-prod-release.aab.sha256` for release managers

The backend `/mobile/releases/{platform}/{channel}/{current_version}` should
expose the same minimum version, recommended version, release notes, checksum,
and download link fields used by the mobile Settings version gate.

## Implemented Files

- `deploy/download/index.html`
- `deploy/download/styles.css`
- `deploy/download/app.js`
- `deploy/download/assets/android-chat-preview.png`
- `deploy/download/privacy.html`
- `deploy/download/mobile-release-manifest.example.json`

## Routing

- Android users: show the signed APK download first until Google Play is ready.
- Google Play: show only when `store_url` exists in the manifest.
- iOS users: show TestFlight/App Store later; until then, keep Android as the
  only primary action.
- Desktop users: link the existing desktop release/download flow later.
- Unknown platforms: show Android APK and support/contact links.

## Android APK Fallback

The APK fallback must show:

- Version name and versionCode.
- Release channel.
- SHA-256 checksum.
- Release notes.
- Install guidance for internal testers.
- Warning to install only from `download.vpsttt.com` or trusted Firebase/Play
  links.

## Required Links

- Direct APK URL.
- SHA-256 checksum URL or visible checksum value.
- Privacy policy.
- Support contact.
- Google Play link later, after Play Console is ready.
- Firebase App Distribution link for internal testers when enabled.

## Acceptance

- The page never links to an unsigned APK.
- The checksum is visible beside the direct APK link.
- The page can be updated from CI artifact metadata without editing app code.
- The page does not expose secrets, signing material, or private tester emails.
- The first viewport gives Android users a clear APK download action.
