# Android Direct Download Plan

This plan finishes the Android-first distribution path before Google Play is
ready. It is scoped to signed Android APK delivery only.

## Goal

- Give internal testers and early Android users one trusted URL:
  `https://download.vpsttt.com`.
- Publish only signed release APKs generated by the M11 workflow.
- Show version, channel, release notes, and SHA-256 checksum on the download
  page.
- Keep Google Play as the later preferred channel without blocking direct APK
  delivery now.

## Implemented Static Page

The static download page lives in `deploy/download/`:

- `index.html`: Android-first download page.
- `styles.css`: responsive visual design.
- `app.js`: reads release metadata from manifest JSON.
- `assets/android-chat-preview.png`: current mobile chat preview screenshot.
- `privacy.html`: minimal privacy/support page for the internal Android channel.
- `mobile-release-manifest.example.json`: example metadata contract.

The page attempts to read manifests in this order:

1. `android/stable/mobile-release-manifest.json`
2. `mobile-release-manifest.json`
3. `mobile-release-manifest.example.json`

## Visual Direction

- First viewport is the actual Android download action, not a marketing hero.
- Use the current mobile chat screenshot as the main product preview.
- Keep one primary action: download signed Android APK.
- Show Google Play only when `store_url` is present in the manifest.
- Surface checksum, version, channel, and release notes near the action.
- Use warning copy for sideload trust: install only from `download.vpsttt.com`,
  Firebase App Distribution, or Google Play links controlled by the team.

## Publish Flow

1. Trigger `.github/workflows/mobile.yml` with `workflow_dispatch`.
2. Confirm the release job produced:
   - `app-prod-release.apk`
   - `app-prod-release.apk.sha256`
   - `app-prod-release.aab`
   - `app-prod-release.aab.sha256`
   - `mobile-release-manifest.json`
3. Upload the signed APK and checksum to:
   `download.vpsttt.com/android/stable/`.
4. Upload the manifest as:
   `download.vpsttt.com/android/stable/mobile-release-manifest.json`.
5. Keep the same manifest fields in the backend
   `/mobile/releases/{platform}/{channel}/{current_version}` response.
6. Open `download.vpsttt.com` on a physical Android device, download the APK,
   install it, log in, and run the M11 smoke route.

## Security Rules

- Do not upload `.jks`, `.keystore`, `key.properties`, Firebase service account
  JSON, tester email lists, or CI secrets to the download host.
- Do not publish debug APKs.
- Do not publish an APK if its SHA-256 does not match the generated checksum.
- Do not replace a published version in place; publish a new versionCode and
  manifest update instead.
- Use HTTPS only.

## Google Play Later

When Google Play is ready, add `store_url` to the manifest. The same page will
show the CH Play action while keeping direct APK as fallback for internal or B2B
distribution.
